/*
 * per compilare: gcc bad_pipeline.c -Wall -O3 -o bad_pipeline
 * per eseguire: ./bad_pipeline <primo programma con argomenti> -x- <secondo comando con argomenti>
 * (es. ./bad_pipeline /bin/echo ciao -x- /bin/cat)
 ...
 * la prima "parte" della pipe e il figlio la "seconda". E' necessario
 * fornire il percorso assoluto dei programmi da eseguire. SE il figlio
 * deve eseguire un programma che necessita interazione col terminale,
 * ad esempio viene lanciato ./bad_pipeline /bin/echo ciao -x-
 * /bin/less, poiche' la bash (nonno, ehm... ancestor) comunica solo col
 * suo rispettivo figlio e non col nipote, al termine di "/bin/echo
 * ciao" eseguito dal padre la shell riprende il terminale, causando
