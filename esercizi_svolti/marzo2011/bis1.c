...
void *routine_tiro(void *arg){
 str_tiratore *tiratore;
  int x, m;
  tiratore=arg;
     sem_getvalue(&archi,&m);
     sem_getvalue(&freccie, &x);
    while(1){
    ...
    tiratore->totale+=tiratore->parziale;//vedo quante freccie sono state usate
         sem_getvalue(&freccie, &x);
         if(x==0)
           pthread_exit(NULL);
  }
...
void *routine_gestore(void *arg){
  int j, x;
   while(1){
     printf("Porto indietro le freccie\n");
    //se sono state usate tutte
       for(j=0; j<I; j++){
       sem_post(&freccie);}
          pthread_mutex_unlock(&bersaglio);
        sem_getvalue(&freccie,&x);
